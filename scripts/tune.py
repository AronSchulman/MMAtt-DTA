...
def run_train(config, data):
    ...
    test_losses = []
    for t in range(start, config["epochs"]):
        print(f"Epoch {t}")
        model.train()
        loss_total = 0
        ...
        session.report({"epoch" : t, "mean_train_loss" : train_loss_to_session, "test_loss" : test_loss, "test_correlation" : spear, "test_rmse" : rmse}, checkpoint=TorchCheckpoint.from_dict({"epoch" : t}))
        wandb.log({f"HEBO {config['protein']} train cosh loss" : mean_train_loss, f"HEBO {config['protein']} test cosh loss" : test_loss, "Test Spearman" : spear, "Test RMSE" : rmse, "Test CI" : cindex})
...
def main():
    ...
    cfg = {
        ...
        "enc_FF_dim_half" : tune.choice([512, 1024, 2048]),
        "att_dropout_half" : tune.uniform(0.0, 0.2),
        "att_layers_half" : 1,
        "enc_FF_dim_full" : tune.choice([512, 1024, 2048]),
        "att_dropout_full" : tune.uniform(0.0, 0.2),
        "att_layers_full" : tune.randint(1, 7),
        "embed_dim" : tune.choice([64, 128, 256, 512]),
        "embed_dim_periodic_compound" : tune.choice([16, 32, 64, 128, 256, 512, 1024]),
        "embed_dim_periodic_protein" : tune.choice([16, 32, 64, 128, 256, 512, 1024]),
        "embed_sigma_compound" : tune.loguniform(1e-2, 1e2),
        "embed_sigma_protein" : tune.loguniform(1e-2, 1e2),
        "hebo_name" : f"hebo_run_{args.protein_class}",
        "wandb": {
            "project" : "DTI_transformer",
            "group" : "Kinase HEBO"
        }
    }
    cfg["protein"] = args.protein_class

    restore_path = f"./HEBO_ray_results/{cfg['hebo_name']}"
    ...
    df_val_x, df_val_y = df_val.drop(["interaction_strength", "pchembl_value"], axis=1), df_val.pchembl_value
    
    data_array = [df_train_x, df_val_x, df_train_y, df_val_y]
    
    
    
    ray_trainer = TorchTrainer(tune.with_parameters(run_train, data=data_array),
    ...
    if tune.Tuner.can_restore(restore_path):
        tuner = tune.Tuner.restore(restore_path, trainable=ray_trainer, resume_errored=True)
        print("Tuner restored from checkpoint!")
    else:
        tuner = tune.Tuner(
            ray_trainer,
    ...
    results = tuner.fit()
    print("Well, the best hyperparameters found were: ", results.get_best_result().config)
